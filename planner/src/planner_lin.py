...
def generate_trajectory(T=13.0, l=1.0, v_max=0.5, omega_max=2, phi_max=np.pi/2-0.01):
    ...
    # Flat outputs (linear trajectory)
    x = xi + (xf - xi) * s
    y = yi + (yf - yi) * s
...
df.to_csv("trajectory_cart_straight.csv", index=False)
